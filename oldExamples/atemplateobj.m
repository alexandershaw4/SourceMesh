...
properties
    ...
    methods
        ...
        function obj = atemplateobj(varargin)
...
%             end
        ...
        function run(obj)
            ...
            data = obj.sort_sourcemodel(data,in);       % Sourcemodel vertices
            
            [mesh,data] = obj.get_mesh(in,data);        % Get Surface
            
            [data,in] = obj.sort_template(data,in);     % Template space? (aal90/78/58)
            
            [mesh,data] = obj.parse_mesh(mesh,in,data); % Mesh to put stuff on
            data.mesh   = mesh;
            
            data = obj.parse_plots(data,in); % Overlays, networks, tracts, nodes, videos etc.
    ...
    %--------------------------------------------------------------------------
    %--------------------------------------------------------------------------
    function data = isopen(obj,data,in)
    ...
    function data = allsides(obj,data,in)
        % a wrapper on atemplate for generating the same plot for the left and
        % right hemis separately in different sublots
    ...
    function data = parse_plots(obj,data,i)
    ...
    function data = parse_labels(obj,i,data)
    ...
    function data = sort_sourcemodel(obj,data,i)
    ...
    function [mesh,data] = parse_mesh(obj,mesh,i,data)
    ...
    if     i.pmesh && ~isfield(i,'T')
           [mesh,data.sourcemodel.pos,h,p] = obj.meshmesh(mesh,i.write,i.fname,i.fighnd,...
    ...
    function [mesh, data] = convert_mesh(obj,mesh,data)
    ...
    if ischar(mesh)
        ...
        switch fe
            ...
            case{'.gz'}
                ...
                gunzip(mesh);
                mesh = strrep(mesh,'.gz','');
                [mesh, data] = obj.convert_mesh(mesh,data);
                return;
    ...
    function [data,i] = sort_template(obj,data,i)
    ...
    if i.template
        atlas = obj.dotemplate(i.model);
        rois  = obj.get_roi_centres(atlas.template_sourcemodel.pos,atlas.all_roi_tissueindex);
        
        atlas.template_sourcemodel.pos = rois;
        atlas = rmfield(atlas,'all_roi_tissueindex');
        
        reg = obj.interp_template(data.sourcemodel,rois);
        atlas.M    = reg.M;
        data.atlas = atlas;
        NM         = atlas.M;
    ...
    function [mesh,data] = get_mesh(obj,i,data)
    ...
    function atlas = dotemplate(obj,model)
    ...
    function atlas = interp_template(obj,atlas,pos)
    ...
    function data = connections(obj,data,A,colbar,write,fname,netcmap,curved)
    ...
    % threshold top n-percent
    if ~isempty(data.network.netthresh)
        fprintf('Thresholding to top %d percent\n',data.network.netthresh*100);
        nthr = data.network.netthresh;
        tA   = triu(A);
        vA   = spm_vec(tA);
        [~,ord] = sort(abs(vA),'descend');
        nc = round(length(ord)*nthr);
        nA = vA*0;
        nA(ord(1:nc)) = vA(ord(1:nc));
        tA = spm_unvec(nA,tA);
        A = tA + triu(tA)';
    end
    
    % threshold top n-percent
    if ~isempty(data.network.netthresh_nonzero)
        fprintf('Thresholding to top %d percent of non-zero\n',data.network.netthresh*100);
        nthr = data.network.netthresh_nonzero;
        tA   = triu(A);
        vA   = spm_vec(tA);
        [~,ord] = sort(abs(vA),'descend');
        nc = round(length(find(vA))*nthr);
        nA = vA*0;
        nA(ord(1:nc)) = vA(ord(1:nc));
        tA = spm_unvec(nA,tA);
        A = tA + triu(tA)';
    ...
    % Edges
    %--------------------------------------------------------------------------
    [node1,node2,strng] = obj.matrix2nodes(triu(A),pos);
    ...
    opacity = obj.rescale(abs(strng2),[.2 1]);
    ...
    function [node1,node2,strng] = matrix2nodes(obj,A,pos)
    ...
    function data = drawnodes(obj,data, N)
    % Node plotter. 
    %
    %
    ...
    function RGB = makecolbar(obj,I,netcmap)
    ...
    function [y,data] = parse_overlay(obj,x,data)
    ...
    if iscell(x)
       ...
       for i = 1:length(x)
           [y{i},data] = obj.parse_overlay(x{i},data);
       end
       return;
    end
    ...
    if ischar(x)
        ...
        switch fe
            ...
            case{'.gz'}
                ...
                [y,data] = obj.parse_overlay(x,data);
                return;
    ...
    %             vol   = ni.img;
                ...
                data.sourcemodel.pos = obj.fit_check_source2mesh(data.sourcemodel.pos,data.mesh);
    ...
    function y = sym_pad_vector(obj,x,n)
    ...
    function [y,data] = vol2surf(obj,vol,data,wb)
    ...
    v = obj.fit_check_source2mesh(v,data.mesh);
    ...
    Ci = obj.compute_reduced_indices(v, fv.vertices);
    ...
    function indices = compute_reduced_indices(obj,before, after)
    ...
    function data = overlay(obj,data,L,write,fname,colbar)
    ...
    if ~isnumeric(L) || (isnumeric(L) && ndims(L)==3)
       ...
       [L,data] = obj.parse_overlay(L,data);
    ...
    function [v,vi] = parcellation_hemispheres(obj,v,vi,cnt)
    ...
    function pos = fit_check_source2mesh(obj,pos,g)
    ...
    function x = killinterhems(obj,x);
    ...
    function newpos = fixmesh(obj,g,pos)
    ...
    function [g,pos,h,p] = meshmesh(obj,g,write,fname,fighnd,a,pos,hemisphere,affine,flip,inflate,checkori,dofillholes,optimise,verb)
    ...
    function data = addlabels(obj,data,V,all_roi_tissueindex,thelabels)
    ...
    function [C,verts] = get_roi_centres(obj,pos,all_roi_tissueindex)
    ...
    function [C,verts] = get_roi_centres0(obj,pos,all_roi_tissueindex)
    % Find centre points of rois
    %
    % starting at 0
    v   = pos;
    ...
    function [Centre,A,B,A0] = spherefit(obj,X)
    ...
    function data = video(obj,data,L,colbar,fpath,tv)
    %
